  ...
  for (let line of lines) {
    ...
    if (headermatch) {
      ...
      if (!listToken) {
        ...
        const newListToken: ListToken = { type: 'list', meta: depth, children: [], parent: listToken };
        listToken.children.push(newListToken);
        listToken = newListToken;
      } else if (depth < listToken.meta) {
        while (listToken.parent.type === 'list' && depth < listToken.meta) {
          listToken = listToken.parent as ListToken;
        }
